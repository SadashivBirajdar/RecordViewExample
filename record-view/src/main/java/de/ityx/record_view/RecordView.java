...
public class RecordView extends RelativeLayout {
  ...
  private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    if (attrs != null && defStyleAttr == -1 && defStyleRes == -1) {
      ...
      if (cancelBounds != -1) setCancelBounds(cancelBounds, false);//don't convert it to pixels since it's already in pixels
  ...
  protected void onActionUp(RecordButton recordBtn) {
    ...
    if (!isLessThanSecondAllowed && isLessThanOneSecond(elapsedTime) && !isSwiped) {
      ...
      if (recordListener != null && !isSwiped) recordListener.onFinish(elapsedTime);
